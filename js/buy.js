...
$(".total").click(function () {
    ...
    if (parseInt($period) <= 0 || parseInt($interest) <= 0) {
        $(".total").closest(".cal-box").find(".result span").text("Error! You must take out a valid car loan.");
    } else if (parseInt($down) >= parseInt($loan)) {
        $(".total").closest(".cal-box").find(".result span").text("The car can be paid in full.");
    } else {
        $remainingBalance = parseInt($loan) - parseInt($down);
        console.log($remainingBalance);
        $months = parseInt($period);
        $monthlyBalance = $remainingBalance / $months;
        $interest = ($monthlyBalance * parseInt($interest)) / 100;
        $monthlyPayment = $monthlyBalance + $interest;
        $(".total").closest(".cal-box").find(".result span").text($monthlyPayment);
    }
});
